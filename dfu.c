...
int32_t dfu_device_exists( libusb_context *usbcontext,
    ...
    for( i = 0; i < devicecount; i++ ) {
        ...
        if( (vendor  == descriptor.idVendor) &&
            ...
            ((bus_number == 0) || ((libusb_get_bus_number(device) == bus_number) && (libusb_get_device_address(device) == device_address))) )
        {
...
struct libusb_device *dfu_device_init( libusb_context *usbcontext, const uint32_t vendor,
                                       const uint32_t product,
                                       const uint32_t bus_number,
                                       const uint32_t device_address,
